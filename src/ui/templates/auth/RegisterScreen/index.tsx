...
const RegisterScreen = () => {
  ...
  return (
    <div className="min-h-screen w-screen overflow-hidden" style={{
      background: 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 50%, #cbd5e1 100%)'
    }}>
      {/* Floating Elements */}
      <div className="absolute top-20 left-10 w-24 h-24 rounded-full blur-xl animate-float opacity-30" style={{ backgroundColor: 'rgba(255, 212, 0, 0.3)' }}></div>
      <div className="absolute bottom-20 right-10 w-32 h-32 rounded-full blur-2xl animate-float-delayed opacity-20" style={{ backgroundColor: 'rgba(0, 60, 195, 0.2)' }}></div>
      <div className="absolute top-1/2 right-20 w-16 h-16 rounded-full blur-lg animate-bounce-soft opacity-25" style={{ backgroundColor: 'rgba(202, 111, 5, 0.4)' }}></div>
      ...
      <div className="hidden md:flex min-h-screen flex-col items-center justify-center p-4 sm:p-6 lg:p-8 relative">
        ...
        <Link href={ROUTES.LANDINGPAGE} className="absolute top-8 left-8 flex items-center gap-2 fade-in-section animate-in z-10 cursor-pointer">
        ...
        <div className="mt-6 w-full max-w-2xl text-center fade-in-section animate-in delay-4">
          <p className="px-6 sm:px-10 text-center text-xs text-gray-500 leading-relaxed">
            ...
            <Link href={ROUTES.LEGAL} className="underline hover:text-blue-600 transition-colors duration-300">
              termos de uso
            </Link>{" "}
            e nossa{" "}
            <Link href={ROUTES.LEGAL} className="underline hover:text-blue-600 transition-colors duration-300">
