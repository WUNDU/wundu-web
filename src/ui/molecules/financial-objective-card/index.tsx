...
const FinancialObjectiveCard: React.FC<FinancialObjectiveCardProps> = ({
  ...
  >
    <div className="flex items-center space-x-4 transition-all duration-300 ease-out">
      ...
      <div>
        <h3 className={`text-base font-semibold ${iconColor} md:text-gray-900 transition-all duration-300 ease-out`}>
          {title}
        </h3>
        <p className="text-xs text-gray-500 mt-1 transition-all duration-300 ease-out">{description}</p>
      </div>
    </div>
  </div>
);
