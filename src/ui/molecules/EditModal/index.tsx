...
import { GoalsService, type GoalPayload, type GoalType } from "@/services/GoalsService";
import { useCategories } from "@/hooks/category/useCategories";
import { getGoalProgress } from "@/hooks/objective/useGoals";
...
const extractCategoryId = (goal: EditModalProps["objective"]) => {
  ...
  return normalizeCategoryId(goal.categoryId ?? goal.category_id ?? goal.category?.id ?? "");
};

const buildInitialFormData = (objective: EditModalProps["objective"]): EditFormState => ({
...
const EditModal: React.FC<EditModalProps> = ({ isOpen, onClose, objective, onUpdated }) => {
  const { showNotification } = useUiStore();
  const { categories, status: categoriesStatus, error: categoriesError } = useCategories();
  const [formData, setFormData] = useState<EditFormState>(() => buildInitialFormData(objective));
  ...
  const resolvedCategoryId = useMemo(() => {
    ...
    if (objective.categoryName && categories.length) {
      const match = categories.find(
        (category) => category.name.toLowerCase() === objective.categoryName?.toLowerCase()
      );
      if (match) {
  ...
  const categoryOptions = useMemo(() => {
    const options = [
      ...
      ...categories.map((category) => ({ value: normalizeCategoryId(category.id), label: category.name })),
    ];

    const selectedCategoryId = resolvedCategoryId;
    const selectedCategoryName =
      categories.find((category) => normalizeCategoryId(category.id) === selectedCategoryId)?.name ??
    ...
    return options;
  }, [categories, isLoadingCategories, objective?.category, objective?.categoryName, resolvedCategoryId]);
      ...
      <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-xl">
        <form className="p-6 md:p-8 space-y-6" onSubmit={handleSubmit}>
          <div className="flex items-center justify-between gap-4">
            <h2 className="text-xl md:text-2xl font-bold text-gray-900">Editar objetivo financeiro</h2>
            <button
              type="button"
              onClick={onClose}
          ...
          {isCompleted && (
            <div className="rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">
              Este objetivo já foi concluído e está disponível apenas para consulta.
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Valor necessário</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Valor arrecadado</label>
              <input
                type="number"
