...
const MovementSection: React.FC<MovementSectionProps> = ({
  ...
  const transactionDocuments = useMemo(() => {
    const normalized = documents.map((doc, index) => {
      ...
      const category =
        (doc as any).category ??
        (doc.type === "image" ? "Imagem" : doc.type === "document" ? "Documento" : "Movimento");
      const description = (doc as any).description ?? "Movimento registrado";
      const timestamp =
        (doc as any).timestamp ?? new Date(Date.now() - index * 60 * 1000).toISOString();
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-xs uppercase font-semibold text-gray-400">Filtrar</p>
              <h3 className="text-lg font-semibold text-gray-800">Personalize a visualização</h3>
            </div>
            <button
              type="button"
