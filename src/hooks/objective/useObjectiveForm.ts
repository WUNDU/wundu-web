...
import { GoalsService, GoalPayload, GoalType } from "@/services/GoalsService";
...
const isDraftEmpty = (data: ObjectiveFormState) =>
  !data.title && !data.description && !data.targetAmount && !data.endDate && !data.categoryId;
...
export const useObjectiveForm = (options?: UseObjectiveFormOptions) => {
  const [form, setForm] = useState<ObjectiveFormState>(() => createDefaultForm());
  const [status, setStatus] = useState<"idle" | "loading" | "success" | "error">("idle");
  const [errorMessage, setErrorMessage] = useState<string>("");
  const [hasDraft, setHasDraft] = useState<boolean>(() => Boolean(getGoalDraft()));
  ...
  const save = async () => {
    ...
    try {
      ...
      setStatus("error");
      setErrorMessage(error instanceof Error ? error.message : "Não foi possível salvar o objetivo.");
      showNotification(
        "error",
        "Erro ao salvar",
