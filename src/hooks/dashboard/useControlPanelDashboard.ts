...
import { useTransactions as useTransactionDocuments } from "@/hooks/transaction/useTransactions";
...
export const useControlPanelDashboard = () => {
  ...
  const incomeEntries = filteredTransactionsRaw.filter((tx) => tx.isIncome).length;
  const expenseEntries = filteredTransactionsRaw.filter((tx) => !tx.isIncome).length;
...
function buildChartData(transactions: NormalizedTransaction[], timeRange: TimeRange) {
