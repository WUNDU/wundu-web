        ...
        <ul className="space-y-1">
          {menuItems.map((item) => (
            <li key={item.name}>
              <Link
                href={item.path}
                className={`flex items-center space-x-3 p-3 rounded-xl font-semibold transition-colors text-sm ${
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="space-y-2">
                {conversations.map((conversation, index) => (
                  <div
                    key={conversation.id}
                    className={`group flex items-start p-3 rounded-lg hover:bg-gray-50 cursor-pointer transition-all duration-300 ${
